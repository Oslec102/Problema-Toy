...
for index, row in limpeza_df.iterrows():
    ...
    for d, dia_abreviado in dias_semana.items():
        nc[a, dia_abreviado, turno] = row[d]
...
f_df = pd.read_excel('ProblemaToy.xlsx',sheet_name=0, usecols=[0,10])
...
x = {(c, a, d, t): model.add_var(var_type=BINARY,name="x({},{},{},{})".format(c, a, d, t)) for c in C_df['Colaborador'].values.tolist() for a in A_df['Area'].values.tolist()  for d, dia_abreviado in dias_semana.items() for t in T_df['Turno'].values.tolist()}
...
for a in A_df['Area'].values.tolist():
    for d, dia_abreviado in dias_semana.items():
        for t in T_df['Turno'].values.tolist():
            if (a, d, t) in nc:   
                model  += xsum(x[(c,a,d,t)] for c in C_df['Colaborador'].values.tolist()) == nc[(a,d,t)]
            else:
                model += xsum(x[c,a,d,t] for c in C_df['Colaborador'].values.tolist()) == 0
